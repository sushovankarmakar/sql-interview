...
INSERT INTO user_events (user_id, created_at, url) VALUES
...
--     SELECT *,
--         lag(ue.created_at) over(PARTITION BY ue.created_at  
--         	ORDER BY ue.user_id ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS next_day
--     FROM user_events ue 
--     WHERE ue.created_at >= '2020-01-01'
--     AND ue.created_at < '2021-01-01'
-- )
-- SELECT *, 
-- 	CASE 
-- 		WHEN next_day IS NULL THEN 1
-- 		WHEN ((date(created_at) - date(next_day)) <= 1) THEN 1
-- 	END AS days_count
-- FROM next_day_details;
    
WITH daily_visits AS (
    -- Get one visit per day per user
    SELECT DISTINCT 
        user_id,
...
SELECT sg.user_id, count(*) AS streak_length
